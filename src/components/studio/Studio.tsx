...
function DocumentaryAnswer({ job, fallback }: { job: Job; fallback: string }) {
  ...
  const eligible = job.model === 'medical' && job.status === 'completed' &&
    ...
    job.result?.generation_policy === 'model_selects_ids_program_copies_contextual_passages';
  const pending = translation !== null && active(translation);
  const ready = translation?.status === 'completed' && translation.result?.translation?.target_language === 'fr' && !!translation.result.answer;
  useEffect(() => {
  ...
  async function translate() {
    ...
    if (ready) { setFrench(!french); return; }
  ...
  return <>
    {eligible && <div className="st-translation-controls">
      <button className="st-secondary" disabled={starting || pending} onClick={() => void translate()}>
        {starting || pending ? 'Traduction en cours…' : displayedFrench ? 'Afficher l’original' : ready ? 'Afficher la traduction française' : 'Traduire en français'}
      </button>
      {pending && <button className="st-text-button" onClick={() => void cancelTranslation()}>Annuler la traduction</button>}
      <span role="status">{displayedFrench ? 'Traduction automatique locale' : pending ? 'L’original reste affiché pendant la traduction.' : 'Texte original'}</span>
    </div>}
    ...
    <div className="st-answer"><ReactMarkdown components={{ img: ({ alt }) => <span>{alt || "Image"}</span>, a: ({ href, children }) => <a href={href} target="_blank" rel="noopener noreferrer">{children}</a> }}>{displayedFrench ? translation!.result!.answer : fallback}</ReactMarkdown></div>
    {displayedFrench ? <p className="st-audit-note">{translation!.result!.audit_notice}</p> : job.result?.audit_notice && <p className="st-audit-note">{job.result.audit_notice}</p>}
  </>;
}
...
const specialist: Record<string, { icon: string; title: string; intro: string; placeholder: string; library: string; suggestions: string[] }> = {
  ...
  medical_v4: { icon: 'book', title: 'Explorer vos documents médicaux.', intro: 'Un mot comme « hernie » suffit pour commencer. Le lecteur recherche dans votre bibliothèque et les publications HAS datées, puis affiche les passages utiles dans leur langue d’origine. Le bouton « Traduire en français » propose une aide de lecture et conserve l’original. Les extraits, leurs conditions et leurs sources restent consultables. Chaque question est indépendante.', placeholder: 'Un terme médical, un point précis ou deux notions à comparer…', library: 'Documents médicaux et publications HAS datées', suggestions: ['hernie', 'Quel rôle du bilan neuropsychologique dans le TDAH chez l’enfant ?', 'Compare la localisation de la cystite et de la pyélonéphrite.'] },
