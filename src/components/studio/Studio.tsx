...
type Source = { archive_member?: string; source_id?: string; surrounding_text?: string; book_id?: number; location_label?: string; source_url?: string; title: string; author?: string; page?: number; text: string; category?: string };
...
function LegalSourceLink({ source }: { source?: Source }) {
  if (!source?.source_url) return null;
  let url: URL;
  try { url = new URL(source.source_url); } catch { return null; }
  if (url.protocol !== 'https:' || !['www.legifrance.gouv.fr', 'opendata.justice-administrative.fr', 'static.data.gouv.fr'].includes(url.hostname)) return null;
  const archive = url.pathname.endsWith('.zip');
  return <div><a href={url.href} target="_blank" rel="noopener noreferrer">{archive ? 'Ouvrir l’archive officielle ↗' : 'Ouvrir la source officielle ↗'}</a>{archive && source.archive_member && <p>Fichier de la décision : {source.archive_member.split('/').pop()}</p>}</div>;
}
