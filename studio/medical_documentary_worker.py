...
from medical_documentary_model import profile,display_source
from models import MODELS

def run(job,save):
    if job['model']!='medical' or job.get('version'):raise ValueError('Profil médical V7 figé requis.')
    root=Path.home()/'Projets'/MODELS['medical']['project'];p=profile(root)
    sys.path.insert(0,str(root));import medical_v7 as reader
